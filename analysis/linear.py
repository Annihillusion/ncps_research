...
log_dir = '/Users/annihillusion/Workflow/ncps_research/data_for_draw/linear'
...
train_loss, val_loss, val_acc, reward = extract_record(log_dir, f'linear.txt')
axes[0].plot(train_loss, label=f'linear')
axes[1].plot(val_loss, label=f'linear')
axes[2].plot(val_acc, label=f'linear')
axes[3].plot(reward, label=f'linear')
axes[0].legend()
axes[1].legend()
axes[2].legend()
axes[3].legend()

plt.savefig('img/linear.png', dpi=200)
plt.show()