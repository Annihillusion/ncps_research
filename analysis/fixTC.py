...
log_dir = '/Users/annihillusion/Workflow/ncps_research/data_for_draw/20&32_fixTC'
...
def sub_draw(num):
    ...
    axes[0].plot(train_loss, label=f'{num}')
    axes[1].plot(val_loss, label=f'{num}')
    axes[2].plot(val_acc, label=f'{num}')
    axes[3].plot(reward, label=f'{num}')

    train_loss, val_loss, val_acc, reward = extract_record(log_dir, f'{num}neurons_ncp_fixTC.txt')
    axes[0].plot(train_loss, label=f'{num} fixTC')
    axes[1].plot(val_loss, label=f'{num} fixTC')
    axes[2].plot(val_acc, label=f'{num} fixTC')
    axes[3].plot(reward, label=f'{num} fixTC')
...
plt.savefig('img/fixTC.png', dpi=200)
plt.show()
